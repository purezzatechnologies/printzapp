...
function Complaints() {
  ...
  const [draftPenalty, setDraftPenalty] = useState("0");
  const [draftStatus, setDraftStatus] = useState<Complaint["status"]>("open");
  ...
  const startEdit = (c: Complaint) => {
    setEditing(c.id);
    setDraftPenalty(String(c.penalty));
    setDraftStatus(c.status);
  };

  const apply = async (id: string) => {
    const penalty = Number(draftPenalty) || 0;
    await setComplaintPenaltyFn({
      data: { id, penalty, status: draftStatus },
    });
    setItems((current) =>
      current.map((c) =>
        c.id === id ? { ...c, penalty, status: draftStatus } : c,
      ),
    );
    setEditing(null);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Complaint Escalations</h2>
        <p className="text-sm text-muted-foreground">
          Review customer disputes, evidence, and apply vendor penalties
        </p>
      </div>
      {uploadError && (
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  Order {c.orderId} · Vendor: {c.vendorName} · Customer: {c.customerName}
                </div>
                {c.detail && (
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_auto_auto]">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    value={draftStatus}
                    onChange={(e) =>
                      setDraftStatus(e.target.value as Complaint["status"])
                    }
                ...
                <div className="flex items-end">
                  <Button onClick={() => apply(c.id)}>Apply</Button>
                </div>
                <div className="flex items-end">
